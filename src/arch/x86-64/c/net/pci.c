...
pci_device_t pci_get_device(uint16_t vendor_id, uint16_t device_id)
{
  ...
  for (uint8_t bus = 0; bus < PCI_BUSES; bus++)
  {
    for (uint8_t num = 0; num < PCI_DEVICES_PER_BUS; num++)
    {
      ...
      uint32_t d_id = pci_read(device, PCI_DEVICE_ID_FIELD_OFFSET, 2); //read data from configuration space of the pci device
      uint32_t v_id = pci_read(device, PCI_VENDOR_ID_FIELD_OFFSET, 2);
...
//procedure reads alligned to 32bit(4bytes) parts. And select corresponding field
uint32_t pci_read(pci_device_t device, uint32_t field, uint8_t size)
{
  //0xFC = 1111 1100b. Note in device struct register_offset field contains 6 bits starting from fird. So we ignore last 2 digits by >> 2 as it is always zero
